...
- ❌ TODO: Update VisitDetailsFragment to include "Add Performance" button for all visits
- ❌ TODO: Create AddPerformanceFragment for adding new performances to existing visits
- ❌ TODO: Implement AddPerformanceViewModel for managing performance creation outside active visits
- ❌ TODO: Add song selection interface to AddPerformanceFragment with same dual-mode functionality
- ❌ TODO: Enable performance timestamp editing for backdated performance entries
- ❌ TODO: Update PerformanceEditFragment to support full performance editing (song, timestamp, key, notes)
- ❌ TODO: Add delete performance functionality with proper confirmation dialogs
- ❌ TODO: Ensure performance statistics update correctly when adding/editing/deleting performances
- ❌ TODO: Test performance management with both active and completed visits

**Phase 11 Status**: Implementation not yet started. Awaiting user authorization to begin development.

#### Technical Requirements - Phase 11
**Performance Management Enhancement:**
- Update VisitDetailsFragment to include "Add Performance" button for all visits (active and completed)
- Create AddPerformanceFragment for comprehensive performance creation outside active visit workflow
- Implement AddPerformanceViewModel with song selection, timestamp editing, key adjustment, and notes capture
- Add song selection interface with configurable search/dropdown modes matching ActiveVisitFragment functionality
- Enable custom timestamp input for backdated performance entries with date/time picker integration

**Enhanced Performance Editing:**
- Expand PerformanceEditFragment to support full performance editing including song selection changes
- Add delete performance functionality with confirmation dialogs and proper cascade handling
- Ensure performance statistics (song.totalPerformances, song.lastPerformed, venue counts) update correctly
- Maintain referential integrity when modifying or deleting performances across all related entities
...
#### User Scenarios Addressed - Phase 11
...
- Backdated performances added with correct chronological ordering within the visit timeframe
- Performance statistics update correctly for songs and venues despite retroactive logging

**Historical Visit Enhancement:**
- User reviews old visit from several months ago and wants to add missing performance details
- User navigates to completed visit → can add forgotten performances with accurate timestamps
- Performance editing allows correcting song selections, key adjustments, and notes for historical accuracy
- Delete functionality enables removing duplicate or incorrect performance entries with proper confirmation
...
- User updates key adjustment information based on better understanding of venue acoustics
- Performance timestamp editing allows correcting entry times for better chronological accuracy
- Enhanced performance notes capture enables adding detailed information after visit completion

#### Implementation Notes - Phase 11
- Build upon existing PerformanceEditFragment and expand functionality for comprehensive editing
- Leverage existing song selection patterns from ActiveVisitFragment for consistent user experience
- Integrate with existing performance statistics system to maintain data integrity during modifications
- Enable full CRUD operations for performances within visit context while preserving referential integrity
- Support both quick performance addition and detailed editing workflows depending on user needs
